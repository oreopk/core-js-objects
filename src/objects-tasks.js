...
function compareObjects(obj1, obj2) {
  for (let i = 0; i < Object.values(obj1).length; i += 1) {
...
function sortCitiesArray(arr) {
  const countries = Array.from(arr);
  for (let i = 0; i < countries.length - 1; i += 1) {
    for (let j = 0; j < countries.length - 1 - i; j += 1) {
      if (
        countries[j].country > countries[j + 1].country ||
        (countries[j].country === countries[j + 1].country &&
          countries[j].city > countries[j + 1].city)
      ) {
        const temp = countries[j];
